...
def replace_booking_outcome(old: Dict[str, Any], new: Dict[str, Any]) -> Dict[str, Any]:
    """
    Strict reducer for booking_outcome. Unlike merge_dicts, this REPLACES entirely
    so a failed booking turn cannot inherit a previous turn's success fields.
    Only keeps old value if new is empty/None (not provided by any agent this turn).
    """
    if new:
        return new
    return old
...
class AgentState(TypedDict):
    ...
    service_request: Annotated[Dict[str, Any], merge_dicts]
    ...
    # ---------------------------------------------------------------
    # BOOKING OUTCOME: Per-turn, strict, non-merged signal block.
    # This is the ONLY authoritative source for supervisor routing
    # decisions after BookingAgent runs. Populated exclusively by
    # BookingAgent; reset to {} at the start of every turn.
    #
    # Schema:
    #   booking_success:     bool   — True only when DB write confirmed
    #   booking_failure:     bool   — True on any failure path
    #   booking_failure_reason: str — Human-readable cause
    #   db_write_confirmed:  bool   — Post-insert find_one succeeded
    #   booking_id:          str    — Inserted booking ObjectId (str)
    # ---------------------------------------------------------------
    booking_outcome: Annotated[Dict[str, Any], replace_booking_outcome]

...
const StatItem = ({ label, value, icon: Icon }) => {
  ...
  return (
    <View className="items-center flex-1">
      <View className={`p-2 rounded-xl mb-1 ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
        <Icon size={18} color={isDark ? '#f1f5f9' : '#000'} />
      </View>
      <Text className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{value}</Text>
      <Text className={`text-[10px] uppercase tracking-wider font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{label}</Text>
    </View>
  );
};
...
const ServiceCard = ({ service, onEdit, onDelete }) => {
  const isDark = useSelector(state => state.orchestration.theme) === 'dark';
  return (
    <Card className={`mb-4 p-4 border shadow-sm overflow-hidden rounded-3xl ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      <View className="flex-row justify-between items-start mb-2">
        <View className="flex-1">
          <Text className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{service.name}</Text>
          <View className="flex-row items-center mt-1">
            <View className={`px-2 py-0.5 rounded-md mr-2 ${isDark ? 'bg-slate-700' : 'bg-slate-900'}`}>
              <Text className="text-[10px] text-white font-bold">{service.service_type}</Text>
            </View>
            <Text className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{service.specialization}</Text>
          </View>
        </View>
        <View className="flex-row space-x-2">
          <TouchableOpacity onPress={onEdit} className={`p-2 rounded-full ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
            <Edit2 size={14} color={isDark ? '#94a3b8' : '#64748b'} />
          </TouchableOpacity>
          <TouchableOpacity onPress={onDelete} className="p-2 bg-red-500/10 rounded-full">
      ...
      <View className={`flex-row items-center justify-between pt-4 border-t ${isDark ? 'border-slate-800' : 'border-slate-50'}`}>
        <View className="flex-row space-x-3">
          <View className="flex-row items-center">
            <MapPin size={12} color={isDark ? '#64748b' : '#64748b'} />
            <Text className={`text-xs ml-1 font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{service.location}</Text>
          </View>
          <View className="flex-row items-center">
            <Clock size={12} color="#64748b" />
            <Text className={`text-xs ml-1 font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{service.experience_years}y Exp</Text>
          </View>
        </View>
        <View className={`px-3 py-1 rounded-full border ${isDark ? 'bg-green-500/10 border-green-500/20' : 'bg-green-50 border-green-100'}`}>
...
const Chip = ({ label, onRemove }) => {
  const isDark = useSelector(state => state.orchestration.theme) === 'dark';
  return (
    <View className={`px-3 py-1.5 rounded-full flex-row items-center mr-2 mb-2 border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
      <Text className={`text-xs font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{label}</Text>
      {onRemove && (

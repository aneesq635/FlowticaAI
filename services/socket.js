  ...
  connect() {
    ...
    try {
      ...
      this.socket.on('request_status_updated', (data) => {
        console.log('[SOCKET EVENT] request_status_updated payload:', JSON.stringify(data));
      ...
      this.socket.on('booking_notification', async (payload) => {
        console.log('[SOCKET EVENT] booking_notification received:', payload);
        // If the notification is for the current user, we should update the unread count in Redux
        if (payload.user_supabase_id === this.currentUserId) {
          try {
            const res = await fetch(`${SOCKET_URL}/api/notifications/${this.currentUserId}`);
            ...
            console.error('[SOCKET EVENT ERROR] Failed to fetch notifications:', err);
          }
        }
      });

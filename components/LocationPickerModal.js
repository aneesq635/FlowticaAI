...
//                         scrollEnabled={!showSuggestions || suggestions.length === 0}
//                         zoomEnabled={!showSuggestions || suggestions.length === 0}
//                         pitchEnabled={!showSuggestions || suggestions.length === 0}
//                         rotateEnabled={!showSuggestions || suggestions.length === 0}
//                     >
//                         {markerCoords && (
//                             <Marker
    ...
    return (
        <Modal visible={visible} animationType="slide" transparent={false}>
            <View style={styles.container}>
                ...
                {/*
                  KEY FIX: Suggestions are now in normal document flow (not absolute).
                  The outer View has a maxHeight so it expands up to 220px and is
                  fully scrollable, then the map shrinks to fill remaining space.
                */}
                {suggestionsVisible && (
                ...
                {/* Map — flex: 1 so it always fills remaining space */}
                <View style={styles.mapContainer}>
                    <MapView
                        style={styles.map}
                        region={region}
                        ...
                        zoomEnabled={!suggestionsVisible}
                        pitchEnabled={!suggestionsVisible}
                        rotateEnabled={!suggestionsVisible}
                    >
                        {markerCoords && (
                            <Marker
                                coordinate={markerCoords}
                    ...
                    {addressPreview ? (
                        <View style={styles.addressBubble}>
                            ...
                            <View style={{ flex: 1, marginLeft: 12 }}>
                                <Text style={styles.addressLabel}>Current Selection</Text>
                                <Text style={styles.addressText} numberOfLines={2}>
                                    {addressPreview}
                                </Text>
                            </View>
...
const styles = StyleSheet.create({
    ...
    // ── Suggestions fix ───────────────────────────────────────────────────────
    // In-flow (not absolute), capped height so it won't swallow the map,
    // border + shadow to feel like a floating card.
    suggestionsWrapper: {
        maxHeight: 220,
        marginHorizontal: 16,
        marginBottom: 4,
        ...
        shadowRadius: 8,
        elevation: 6,
        overflow: 'hidden', // keeps rounded corners on Android
    },
    // ─────────────────────────────────────────────────────────────────────────
